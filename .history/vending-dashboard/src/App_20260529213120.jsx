...
export default function App() {
  ...
  // Realtime DB listeners — only when logged in
  useEffect(() => {
    if (!user) return;
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside style={{ width:'240px', flexShrink:0, background:'#fff', borderRight:'1.5px solid #e2e8f0', padding:'24px 14px', display:'flex', flexDirection:'column', gap:'4px', position:'fixed', top:0, left:0, height:'100vh', zIndex:100, overflowY:'auto' }}>
        ...
        {navItems.map(item => (
          <button key={item.id} onClick={() => setPage(item.id)} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', borderRadius:'10px', border:'none', cursor:'pointer', fontFamily:"'Sora',sans-serif", fontSize:'13px', fontWeight: page === item.id ? '700' : '500', background: page === item.id ? '#ede9fe' : 'transparent', color: page === item.id ? '#7c3aed' : '#64748b', transition:'all 0.18s ease', textAlign:'left', width:'100%' }}
        ...
        <div style={{ marginTop:'auto', display:'flex', flexDirection:'column', gap:'12px' }}>
          ...
          {/* Logout button */}
          <button onClick={handleLogout} disabled={loggingOut} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'10px 12px', borderRadius:'10px', border:'1.5px solid #fee2e2', background:'#fff', color:'#ef4444', fontFamily:"'Sora',sans-serif", fontSize:'13px', fontWeight:'600', cursor:'pointer', transition:'all 0.18s', width:'100%' }}
