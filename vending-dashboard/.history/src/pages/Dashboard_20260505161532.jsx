...
export default function Dashboard({ inventory, stats, lowStockCount, setPage }) {
  ...
  return (
    <div>
      <style>{`
        .stat-card { background:#fff; border-radius:18px; padding:24px; border:1.5px solid #e2e8f0; transition:transform .2s,box-shadow .2s,border-color .2s; animation:fadeUp .45s ease both; cursor:default; }
        .stat-card:hover { transform:translateY(-4px); box-shadow:0 16px 48px rgba(0,0,0,.08); border-color:#c7d2fe; }
        .slot-row { background:#fff; border-radius:14px; padding:16px 20px; border:1.5px solid #e2e8f0; display:flex; align-items:center; gap:16px; transition:all .2s; animation:fadeUp .45s ease both; }
        .slot-row:hover { border-color:#a5b4fc; box-shadow:0 8px 24px rgba(0,0,0,.06); transform:translateX(3px); }
        .slot-row.low { border-color:#fca5a5; background:linear-gradient(90deg,#fff 70%,#fff5f5); }
        .view-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 16px; border-radius:8px; border:none; cursor:pointer; font-family:'Sora',sans-serif; font-size:12px; font-weight:600; background:#f1f5f9; color:#475569; transition:all .18s; }
        .view-btn:hover { background:#ede9fe; color:#7c3aed; }
      `}</style>

      {/* Header */}
      <div style={{ marginBottom: '32px', animation: 'fadeUp .4s ease' }}>
          ...
          <div key={i} className="stat-card" style={{ animationDelay: s.delay }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div className="mono" style={{ fontSize: '30px', fontWeight: '700', color: '#0f172a', marginTop: '8px', letterSpacing: '-1px' }}>{s.value}</div>
              </div>
              <div style={{ width: '40px', height: '40px', borderRadius: '11px', background: s.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: s.color }}>
                {s.icon}
              </div>
            </div>
          </div>
      ...
      {/* Low stock banner */}
      {lowStockCount > 0 && (
        <div style={{ background: 'linear-gradient(90deg,#fff5f5,#fff)', border: '1.5px solid #fca5a5', borderLeft: '4px solid #ef4444', borderRadius: '14px', padding: '16px 20px', marginBottom: '28px', display: 'flex', alignItems: 'center', gap: '14px', animation: 'fadeUp .4s ease' }}>
          <div style={{ width: '36px', height: '36px', background: '#fee2e2', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444', flexShrink: 0 }}>{Icons.alert}</div>
          <div style={{ flex: 1 }}>
          ...
          <button onClick={() => setPage('inventory')} className="view-btn" style={{ background: '#fee2e2', color: '#ef4444' }}>
            Manage {Icons.arrow}
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {slots.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', background: '#fff', borderRadius: '14px', border: '1.5px dashed #e2e8f0' }}>
        ...
        {slots.map(([id, item], idx) => {
          ...
          return (
            <div key={id} className={`slot-row ${isLow ? 'low' : ''}`} style={{ animationDelay: `${idx * 0.06}s` }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: isLow ? '#fee2e2' : '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', color: isLow ? '#ef4444' : '#7c3aed', fontWeight: '800', fontSize: '12px', flexShrink: 0, fontFamily: "'JetBrains Mono',monospace" }}>
                {id.replace('slot', 'S')}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: '600', fontSize: '13px', color: '#1e293b', marginBottom: '6px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</div>
                <StockBar stock={item.stock} capacity={item.capacity} />
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div className="mono" style={{ fontSize: '13px', fontWeight: '700', color: isLow ? '#ef4444' : '#1e293b' }}>{item.stock}/{item.capacity}</div>
                <div style={{ fontSize: '11px', color: '#94a3b8' }}>{pct}%</div>
              </div>
              <div style={{ fontSize: '13px', fontWeight: '700', color: '#7c3aed', fontFamily: "'JetBrains Mono',monospace", flexShrink: 0 }}>₱{item.price}</div>
